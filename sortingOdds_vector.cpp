...
public:
  vector<int> sortArray(vector<int> array) {
    ...
    vector<int> indicesOfOdd; // uset to store the indices of odd numbers
    for (int i = 0; i < array.size(); i++) {
      if (array[i] % 2 != 0) {
        odds.push_back(array[i]);
        indicesOfOdd.push_back(i);
      } else {
        continue;
      }
    }
    ...
    int counter = 0;
    while (counter < indicesOfOdd.size()) {
      array[indicesOfOdd[counter]] = odds[counter];
      counter++;
    }
